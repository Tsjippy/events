...
const Edit = ({ setAttributes, attributes }) => {
  ...
  useEffect(() => {
    if (meta["tsjippy_eventdetails"] != "" && meta["tsjippy_eventdetails"] != undefined) {
      let data = JSON.parse(meta["tsjippy_eventdetails"]);
    ...
    if (key == "startdate") {
      ...
      if (subkey == "excludedates" || subkey == "includedates") {
        ...
        if (newEvent["repeat"][subkey].includes(date)) {
          newEvent["repeat"][subkey] = newEvent["repeat"][subkey].filter(
            (el) => el != date,
          );
          message = `removed`;
        } else {
  ...
  const { users, userResolved } = useSelect(
    (select) => {
      ...
      return {
        ...
        userResolved: select(coreDataStore).hasFinishedResolution("getUsers", [
          query,
        ]),
      };
    },
  ...
  const { locations, locationResolved } = useSelect(
    (select) => {
      ...
      return {
        locations: select(coreDataStore).getEntityRecords(...args),
  ...
  const CreateExcludeDatesControls = () => {
    ...
    return eventData.repeat.excludedates.map((filter, index) => (
      <DatePicker
        ...
        value={filter}
        onChange={(value) => {
          updateMetaValue("repeat-excludedates", value, index);
        }}
      />
    ));
  };
  ...
  const WeekRepeatDetails = () => {
    ...
    let checkboxes = ["First", "Second", "Third", "Fourth", "Fifth", "All"].map(
      (t) => {
        return (
          <CheckboxControl
            label={__(t)}
            ...
            onChange={(selected) =>
              updateMetaValue("repeat-weeks", selected, t)
            }
          />
        );
      },
    ...
    return (
      <>
        ...
        <br></br>
        <div class="flex week-selector">{checkboxes}</div>
        <br></br>
      </>
    );
  };
  ...
  const MonthRepeatDetails = () => {
    ...
    return (
      <>
        ...
        <RadioGroup
          label="Width"
          onChange={(value) => updateMetaValue("repeat-datetype", value)}
          checked={eventData["repeat"]["datetype"]}
        >
          <Radio value="samedate">{__("On the same day")}</Radio>
          <Radio value="patterned">
            {__(`On the ${weekWord} ${weekDay} of the month`)}
          </Radio>
          <Radio value="last">
            {__(`On the last ${weekDay} of the month`)}
          </Radio>
  ...
  const MonthRepeatPattern = () => {
    ...
    return (
      <>
        ...
        <br></br>
        <div class="flex month-selector">{checkboxes1}</div>
        <div class="flex month-selector">{checkboxes2}</div>
        <div class="flex month-selector">{checkboxes}</div>
        <br></br>
      </>
    );
  };
  ...
  const CreateIncludeDatesControls = () => {
    ...
    let datePicker = (
      <DatePicker
        currentDate={null}
        onChange={(value) => {
          updateMetaValue("repeat-includedates", value);
        }}
      />
    );
      ...
      <>
        {eventData.repeat.includedates.map((filter, index) => (
          <DatePicker
            ...
            value={filter}
            onChange={(value) => {
              updateMetaValue("repeat-includedates", value, index);
            }}
          />
        ))}
        {datePicker};
  ...
  const StopDetails = () => {
    if (
      eventData["repeat"]["stop"] == undefined ||
      eventData["repeat"]["stop"] == "never"
    ) {
      return "";
    }
    ...
    return (
      <>
        ...
        <div id="event-repeat-details">
          ...
          <RadioGroup
            label="Width"
            onChange={(value) => updateMetaValue("repeat-type", value)}
            checked={eventData["repeat"]["type"]}
          >
          ...
          <DatePicker
            currentDate={null}
            onChange={(value) =>
              updateMetaValue("repeat-excludedates", value, 0)
            }
            __nextRemoveHelpButton
            __nextRemoveResetButton
          />
  ...
  const EventDetails = () => {
    if (
      (eventData["endtime"] == undefined &&
        (eventData["allday"] == undefined || !eventData["allday"])) ||
    ...
    return (
      <>
        ...
        <SearchControl onChange={setUserSearchTerm} value={userSearchTerm} />
        ...
        <Button
          ...
          onClick={(event) =>
            updateMetaValue("isrepeated", !eventData["isrepeated"])
          }
        >
          Repeat this event
        </Button>
    ...
    const TimePickers = () => {
      ...
      let startTime =
        eventData["starttime"] == undefined
          ...
          : eventData["starttime"];
      let endTime =
        eventData["endtime"] == undefined ? "00:00:00" : eventData["endtime"];
