...
function updateDayNameDisplay() {
  let startDate = new Date(
    document.querySelector('[name="event[startdate]"]').value,
  );
  let weekDays = [
    "sunday",
    "monday",
  ...
  document
    .querySelectorAll(".dayname")
    .forEach((el) => (el.textContent = weekDay));
  document
    .querySelectorAll(".weekword")
    .forEach((el) => (el.textContent = weekWord));

  // get the last day of the month
  let lastDay = new Date(startDate.getFullYear(), startDate.getMonth() + 1, 0);

  document
    .querySelectorAll(".last-day-of-month")
    .forEach((el) => el.classList.add("hidden"));
  document
    .querySelectorAll(".last-dayname-of-month")
    .forEach((el) => el.classList.add("hidden"));
...
function repeatTypeChosen(target) {
  ...
  parent
    ...
    .forEach((el) => el.classList.replace("hide", "hidden"));
  ...
  switch (target.value) {
    case "daily":
      //show
      parent
        .querySelectorAll(".repeatinterval, .days")
        .forEach((el) => el.classList.replace("hidden", "hide"));
      parent.querySelector("#repeattype").textContent = "days";
      break;
    case "weekly":
      parent
        .querySelectorAll(".repeatinterval, .weeks")
        .forEach((el) => el.classList.replace("hidden", "hide"));
      parent.querySelector("#repeattype").textContent = "week(s)";
      break;
    case "monthly":
      parent
        .querySelectorAll(".repeatdatetype, .months")
        .forEach((el) => el.classList.replace("hidden", "hide"));
      parent.querySelector("#repeattype").textContent = "month(s)";
      break;
    case "yearly":
      parent
        .querySelectorAll(".repeatdatetype, .years")
        .forEach((el) => el.classList.replace("hidden", "hide"));
      parent.querySelector("#repeattype").textContent = "year(s)";
      break;
    case "custom_days":
      parent
        .querySelectorAll(".custom-dates-selector")
        .forEach((el) => el.classList.replace("hidden", "hide"));
      break;
  }
}
...
function allDayClicked(target) {
  let startTime = target
    .closest(".event")
    .querySelector('[name="event[starttime]"]');
  let endTime = target
    .closest(".event")
    .querySelector('[name="event[endtime]"]');
  let endDate = target
    .closest(".event")
    .querySelector('[name="enddate-label"]');
...
function startDateChanged(target) {
  ...
  let endDate = target
    .closest(".event")
    .querySelector('[name="event[enddate]"]');
  ...
  document.querySelectorAll('.weeks [name="event[repeat][weeks][]"]')[
    montWeek
  ].checked = true;
}
...
function repeatStopOptionChosen(target) {
  ...
  target
    .closest(".event")
    .querySelectorAll(".repeat-type-option")
    .forEach((el) => {
  ...
  target
    .closest(".repeat-type-option")
    .querySelector(".repeat-type-option_specifics")
...
document.addEventListener("click", (event) => {
  ...
  if (target.name == "enable-event-repeat") {
    ...
    document
      .querySelectorAll(".repeat-type-option_specifics:not(.hidden)")
      .forEach((el) => el.classList.add("hidden"));
    target
      .closest(".repeat-type-option")
      .querySelector(".repeat-type-option_specifics")
